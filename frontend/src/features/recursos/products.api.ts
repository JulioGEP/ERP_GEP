...
export async function fetchProducts(): Promise<Product[]> {
  const json = (await requestJson<ProductListResponse>(`/products`)) ?? {};
...
export async function updateProduct(productId: string, payload: ProductUpdatePayload): Promise<Product> {
  ...
  const json =
    (await requestJson<ProductMutationResponse>(`/products/${encodeURIComponent(productId)}`, {
      method: 'PATCH',
...
export async function syncProducts(): Promise<ProductSyncSummary | null> {
  const json =
    (await requestJson<ProductSyncResponse>(`/products-sync`, {
      method: 'POST',
    })) ?? {};
