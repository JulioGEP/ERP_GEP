// frontend/src/features/recursos/TrainerAvailabilitySection.tsx
...
type TrainerAvailabilitySectionProps = {
  trainerId?: string | null;
};

export function TrainerAvailabilitySection({ trainerId }: TrainerAvailabilitySectionProps) {
  const queryClient = useQueryClient();
  const currentYear = useMemo(() => new Date().getFullYear(), []);
  const [selectedYear, setSelectedYear] = useState(currentYear);

  const baseQueryKey = useMemo(
    () => ['trainer', 'availability', trainerId ?? 'self'] as const,
    [trainerId],
  );
  const queryKey = useMemo(() => [...baseQueryKey, selectedYear] as const, [baseQueryKey, selectedYear]);

  const availabilityQuery = useQuery({
    queryKey,
    queryFn: () => fetchTrainerAvailability({ year: selectedYear, trainerId }),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const mutation = useMutation({
    mutationFn: (update: TrainerAvailabilityOverride) =>
      updateTrainerAvailability([{ date: update.date, available: update.available }], { trainerId }),
    onMutate: async (update) => {
      const previous = availabilityQuery.data;
      if (!previous) return { previous: null };

      await queryClient.cancelQueries({ queryKey });

      const map = new Map<string, boolean>();
      for (const entry of previous.overrides) {
        map.set(entry.date, entry.available);
      }
    ...
    onSuccess: (data) => {
      queryClient.setQueryData([...baseQueryKey, data.year], data);
    },
  });
    ...
    <Stack gap={4} className="trainer-availability-page">
      ...
      {availabilityQuery.isLoading ? (
        ...
        <Alert variant="danger" className="d-flex flex-column flex-md-row align-items-md-center gap-3">
          <div>
            <div className="fw-semibold">No se pudo cargar la disponibilidad.</div>
