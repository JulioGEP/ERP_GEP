...
export type ActuacionesPreventivosMonthlyKpi = {
  month: string;
  ...
  promedioActividadDia: number;
};

export type ActuacionesPreventivosLogItem = {
  id: string;
  presupuesto: string;
  cliente: string;
...
export type ActuacionesPreventivosDashboardResponse = {
  monthlyKpis: ActuacionesPreventivosMonthlyKpi[];
  logs: ActuacionesPreventivosLogItem[];
};

export type ActuacionesPreventivosDashboardFilters = {
  startDate?: string;
  endDate?: string;
};

export async function fetchActuacionesPreventivosDashboard(
  filters: ActuacionesPreventivosDashboardFilters = {}
): Promise<ActuacionesPreventivosDashboardResponse> {
  const params = new URLSearchParams();
  ...
  const query = params.toString();
  const url = query.length
    ? `/reporting-actuaciones-preventivos?${query}`
    : '/reporting-actuaciones-preventivos';

  return getJson<ActuacionesPreventivosDashboardResponse>(url);
}
