...
function resolveServiceRate(record: TrainerExtraCostRow | null, type: 'formacion' | 'preventivo'): number {
...
export const handler = createHttpHandler(async (request) => {
  ...
  if (!trainer) {
    return errorResponse('NOT_FOUND', 'No se ha encontrado un formador asociado al usuario.', 404);
  }
  ...
  const sessionRows = (await prisma.sesion_trainers.findMany({
    where: sessionWhere,
    select: {
      sesion_id: true,
      sesiones: {
  ...
  const sessionIds = sessionRows
    .map((row) => row.sesion_id || row.sesiones?.id)
  ...
  for (const record of extraCostRecords) {
    if (record.session_id) {
      extraCostMap.set(`session:${record.session_id}`, record);
    }
    if (record.variant_id) {
      extraCostMap.set(`variant:${record.variant_id}`, record);
    }
  }

  const items: TrainerSessionHours[] = [];

  for (const row of sessionRows) {
    const sessionId = row.sesion_id || row.sesiones?.id;
  ...
  for (const row of variantRows) {
    ...
    const extraCostRecord = extraCostMap.get(`variant:${row.id}`) ?? null;
    ...
    items.push({
      ...
      sessionDate: formatSessionDate(row.date ? new Date(row.date) : null),
      totalHours: roundToTwoDecimals(hours),
      serviceCost: roundToTwoDecimals(serviceCost),
      extraCost: roundToTwoDecimals(extraCost),
      payrollCost: roundToTwoDecimals(serviceCost + extraCost),
  ...
  items.sort((a, b) => {
    ...
    return a.sessionName?.localeCompare(b.sessionName ?? '') ?? 0;
  });

  const totalSessions = items.length;
  const totalHours = roundToTwoDecimals(items.reduce((acc, item) => acc + item.totalHours, 0));
  const totalServiceCost = roundToTwoDecimals(items.reduce((acc, item) => acc + item.serviceCost, 0));
  const totalExtraCost = roundToTwoDecimals(items.reduce((acc, item) => acc + item.extraCost, 0));
  const totalPayrollCost = roundToTwoDecimals(items.reduce((acc, item) => acc + item.payrollCost, 0));

  return successResponse({
    trainer: {
      id: trainer.trainer_id,
      name: trainer.name,
      lastName: trainer.apellido,
    },
    items,
    summary: {
      totalSessions,
