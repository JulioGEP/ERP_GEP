...
export async function withDatabaseFallback<T>(operation: (client: PrismaClient) => Promise<T>): Promise<T> {
  try {
    ...
    console.warn('Primary Prisma connection failed; retrying with DIRECT_URL.', {
