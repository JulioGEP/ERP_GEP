...
const ORG_COMPANY_TYPE_FIELD_KEY = String(
  process.env.LEAD_FORM_PIPE_ORG_COMPANY_TYPE_FIELD_KEY ?? '8a65e9b780cbab3f08ccc8babe92a290fb79f216',
).trim();
const ORG_ACQUISITION_CHANNEL_FIELD_KEY = String(
  process.env.LEAD_FORM_PIPE_ORG_ACQUISITION_CHANNEL_FIELD_KEY ?? '6eb20e6b912f055c127241c9012f20a8223637f6',
).trim();
...
function buildOrganizationPayload(lead: NormalizedLeadForm) {
  ...
  if (isOpenTrainingBudgetLead(lead)) {
    ...
    payload[ORG_COMPANY_TYPE_FIELD_KEY] = toPipedriveOptionValue(OPEN_TRAINING_INDIVIDUAL_COMPANY_TYPE_OPTION_ID);
    payload[ORG_ACQUISITION_CHANNEL_FIELD_KEY] = toPipedriveOptionValue(OPEN_TRAINING_INDIVIDUAL_CHANNEL_OPTION_ID);
  }
