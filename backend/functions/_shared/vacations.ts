...
export const VACATION_TYPES = new Set(['V', 'L', 'A', 'T', 'M', 'H', 'F', 'R', 'P', 'I', 'N']);

export type VacationCounts = Record<'V' | 'L' | 'A' | 'T' | 'M' | 'H' | 'F' | 'R' | 'P' | 'I' | 'N', number>;
...
export async function buildVacationPayload(
  ...
  const counts: VacationCounts = { V: 0, L: 0, A: 0, T: 0, M: 0, H: 0, F: 0, R: 0, P: 0, I: 0, N: 0 };
  ...
  const enjoyed =
    counts.V + counts.L + counts.A + counts.M + counts.H + counts.F + counts.R + counts.P + counts.I + counts.N;
