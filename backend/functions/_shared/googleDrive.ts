...
import { resolveSessionNumber } from "./sessions";
...
const USERS_FOLDER_NAME = "Equipo GEP Group";
  ...
  do {
    const data = await driveFilesList({
      ...
      q: [
        ...
        "trashed = false",
        params.mimeType ? `mimeType = '${params.mimeType}'` : null,
      ]
        .filter(Boolean)
        .join(" and "),
      fields:
        "nextPageToken, files(id,name,mimeType,webViewLink,createdTime,modifiedTime,parents,trashed)",
    });
...
function parseDealFolderId(name: string | null | undefined): string | null {
  if (!name) return null;
...
function parseSessionNumberFromFolder(name: string | null | undefined): string | null {
  if (!name) return null;
  const trimmed = name.trim();
...
function inferTrainerDocumentTypeFromFileName(name: string): string {
...
function buildDriveFileWebViewLink(file: { id?: string | null; webViewLink?: string | null }): string | null {
  if (file?.webViewLink && typeof file.webViewLink === "string") {
...
function inferUserFolderMatch(params: { folderName: string; users: Array<{ id: string; name: string | null }> }): {
  userId: string | null;
} {
  const normalizedFolder = sanitizeName(params.folderName || "");
  for (const user of params.users) {
    const candidate = sanitizeName(user?.name || "");
    if (candidate && candidate === normalizedFolder) {
...
export async function importGoogleDriveDocumentsIntoErp(): Promise<{
  dealDocuments: number;
  sessionDocuments: number;
  trainerDocuments: number;
  ...
  if (!baseFolderId) {
    throw new Error("No se encontró la carpeta base en Drive");
  }

  const organizationFolders = await listFolderChildren({
  ...
  for (const organizationFolder of organizationFolders) {
    const dealFolders = await listFolderChildren({
    ...
    for (const dealFolder of dealFolders) {
      const dealId = parseDealFolderId(dealFolder.name);
      if (!dealId) continue;

      const deal = await prisma.deals.findUnique({
        where: { deal_id: dealId },
        include: { sesiones: true, organizations: { select: { name: true } } },
      });

      if (!deal) continue;

      const sessionNumberMap = new Map<string, any>();
      for (const session of deal.sesiones ?? []) {
        const number = await resolveSessionNumber(prisma, session);
        sessionNumberMap.set(number, session);
      }

      const dealChildren = await listFolderChildren({ folderId: dealFolder.id, driveId });
      for (const child of dealChildren) {
        if (child.mimeType === "application/vnd.google-apps.folder") {
          const sessionNumber = parseSessionNumberFromFolder(child.name);
          if (!sessionNumber) continue;
          const session = sessionNumberMap.get(String(sessionNumber));
          if (!session) continue;

          const sessionFiles = await listFolderChildren({ folderId: child.id, driveId });
          for (const sessionFile of sessionFiles) {
            if (sessionFile.mimeType === "application/vnd.google-apps.folder") {
              const nestedFiles = await listFolderChildren({ folderId: sessionFile.id, driveId });
        ...
        const extension = (child.name || "").split(".").pop() || "bin";
        await prisma.deal_files.upsert({
          where: { id: child.id },
          create: {
            id: child.id,
            deal_id: deal.deal_id,
            file_name: child.name ?? child.id,
            file_type: extension,
            drive_file_name: child.name ?? child.id,
            drive_web_view_link: buildDriveFileWebViewLink(child),
            added_at: child.modifiedTime ? new Date(child.modifiedTime) : new Date(),
          },
          update: {
            file_name: child.name ?? child.id,
            file_type: extension,
        ...
        summary.dealDocuments += 1;
      }
    }
  }

  const trainersRootId = await findFolder({
    name: TRAINERS_FOLDER_NAME,
    parentId: driveId,
    driveId,
  });

  if (trainersRootId) {
    const trainers = await prisma.trainers.findMany({ select: { trainer_id: true, name: true, apellido: true } });
    const trainerFolders = await listFolderChildren({
      folderId: trainersRootId,
      driveId,
      mimeType: "application/vnd.google-apps.folder",
    });
    ...
    for (const trainerFolder of trainerFolders) {
      ...
      const trainerFiles = await listFolderChildren({ folderId: trainerFolder.id, driveId });
      for (const trainerFile of trainerFiles) {
        if (trainerFile.mimeType === "application/vnd.google-apps.folder") continue;
        const documentType = inferTrainerDocumentTypeFromFileName(trainerFile.name || "");
        await prisma.trainer_documents.upsert({
          where: { id: trainerFile.id },
          create: {
            id: trainerFile.id,
            trainer_id: matchedTrainer.trainer_id,
            document_type: documentType,
            file_name: trainerFile.name ?? trainerFile.id,
            original_file_name: trainerFile.name ?? trainerFile.id,
            mime_type: trainerFile.mimeType ?? null,
            drive_file_id: trainerFile.id,
            drive_file_name: trainerFile.name ?? trainerFile.id,
            drive_web_view_link: buildDriveFileWebViewLink(trainerFile),
            uploaded_at: trainerFile.modifiedTime ? new Date(trainerFile.modifiedTime) : new Date(),
          },
          update: {
            document_type: documentType,
            file_name: trainerFile.name ?? trainerFile.id,
        ...
        summary.trainerDocuments += 1;
      }
    }
  }

  const usersRootId = await findFolder({ name: USERS_FOLDER_NAME, parentId: driveId, driveId });
  if (usersRootId) {
    const users = await prisma.users.findMany({ select: { id: true, name: true } });
    const userFolders = await listFolderChildren({ folderId: usersRootId, driveId, mimeType: "application/vnd.google-apps.folder" });

    for (const userFolder of userFolders) {
      const { userId } = inferUserFolderMatch({ folderName: userFolder.name || "", users });
      if (!userId) continue;

      const userFiles = await listFolderChildren({ folderId: userFolder.id, driveId });
      for (const userFile of userFiles) {
        if (userFile.mimeType === "application/vnd.google-apps.folder") continue;

        await prisma.user_documents.upsert({
          where: { id: userFile.id },
          create: {
            id: userFile.id,
            user_id: userId,
            title: userFile.name ?? userFile.id,
            file_name: userFile.name ?? userFile.id,
            mime_type: userFile.mimeType ?? null,
            drive_file_id: userFile.id,
            drive_folder_id: userFolder.id,
            drive_web_view_link: buildDriveFileWebViewLink(userFile),
